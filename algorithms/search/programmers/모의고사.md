...
def solution(answers):
    ...
    users = [[1,2,3,4,5],[2,1,3,4,5],[3,1,2,4,5]] # 수포자 찍는 순서
    user_cnt = [0,0,0] # 맞은 갯수

    for j in range(len(answers)):
        if j % 2 == 0:
            user2 = users[1][0]
            user3 = users[2].pop(0)
            users[2].append(user3)

        else:
            user2 = users[1].pop(1)
            users[1].append(user2)

        if users[0][j % 5] == answers[j]:
            user_cnt[0] += 1
        if user2 == answers[j]:
            user_cnt[1] += 1
        if user3 == answers[j]:
    ...
    ans_cnt = max(user_cnt)

    for i in range(3):
        if ans_cnt == user_cnt[i]:
            answer.append(i+1)
